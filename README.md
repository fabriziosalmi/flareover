...
| Object storage (R2/S3 →) | MinIO self-hosted, or managed EU S3 — **Scaleway** (`storage --dest scaleway`) / **OVHcloud** (`--dest ovh`) / **Contabo** (`--dest contabo`, EU region `eu2`) — buckets + versioning + lifecycle + CORS; rclone data copy |
